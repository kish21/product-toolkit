...
- **Within-tenant authz (not just cross-tenant):** `org_id`/RLS stops cross-tenant leaks but NOT one user seeing another user's resources inside the same org. Is there a per-resource access check (owner/collaborator/role)? **Audit EVERY resource-returning endpoint individually — including streaming ones.** SSE/WebSocket/`StreamingResponse` handlers are the classic gap: they routinely do an org-scoped fetch but SKIP the per-resource authz guard the REST endpoints call (grep for `StreamingResponse`/`text/event-stream`/`async def *stream*` and confirm each calls the same `require_*_access` as its REST siblings). A "✅ guarded" claim from a code-map is not proof — read the actual handler lines.
- **Service-role/admin DB clients bypass RLS on EVERY query — is each such read explicitly scoped in the query itself?** (grep for `service_role`/`SERVICE_ROLE_KEY`/admin-client constructors, then read each query they run) Row policies do not execute for the service key, so a service-role read needs its scope re-stated inline (a tenant filter, or an explicit `tenant_id IS NULL` for global rows) and ideally test-pinned. A service-role read with no explicit scope is a cross-tenant leak waiting for its first row. Also check any client-supplied fragment is validated BEFORE reaching a raw query-builder string (a PostgREST `.or_(...)` is an injection surface exactly like SQL).
- **Role set is one source of truth:** the valid-role list should live in ONE place (e.g. an auth module constant) reused by the JWT validator, the DB CHECK constraint, and any role-assignment endpoint. Grep for duplicated role literals (`'admin','user',...`) across files — copies drift. Read-only/compliance roles (e.g. `auditor`) must be EXCLUDED from any write/launch allow-list, not just denied per-endpoint.
