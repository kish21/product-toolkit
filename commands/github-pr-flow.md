...
| Local gate / pre-push hook fails on a test unrelated to your diff — typically `ModuleNotFoundError` for a package the project definitely depends on | The shell's ACTIVE interpreter is another project's virtualenv, and the gate script calls bare `python`. Your diff is fine; the environment picked the wrong Python | Confirm before blaming anything: `python -c "import sys; print(sys.prefix)"`. Then run the gate AND the push with the project's own venv first on `PATH`: `env VIRTUAL_ENV=<venv> PATH=<venv>/Scripts:$PATH git push`. Cross-check by stashing to a clean tree — if it still fails, it was never yours |
| A `PATH` override is silently ignored on Windows/Git Bash — `command -v python` keeps finding the OLD one even though you prepended the right directory | You put a Windows drive-letter path (`c:/Users/...`) INTO `PATH`. `PATH` is colon-separated, so the colon in `c:` splits the entry into two junk fragments and your override never exists | Use POSIX form for anything prepended to `PATH`: `/c/Users/...`. Convert drive-letter vars first (`cygpath -u "$WINPATH"`). ⚠️ Verify the override actually TOOK (`command -v python`) before concluding the environment is broken — this failure mode looks exactly like a hostile shell |
| Every CI job fails in ~2 seconds with no logs, `log not found` from `gh run view --log-failed` | Not a code failure — the jobs were never started. Read the run's ANNOTATIONS: usually GitHub Actions billing (failed payment / spending limit) or exhausted minutes | `gh run view <run-id>` and read the `ANNOTATIONS` block. A billing block is the repo owner's to clear in Billing & plans; do not touch the diff, and say plainly that CI did not run rather than reporting it as red |
| `gh pr merge` refuses with *"add the `--auto` flag / add the `--admin` flag"* even though CI is green | The ruleset requires a PR **review**, and on a solo-owned repo you cannot approve your own PR — so the PR sits `MERGEABLE`/`CLEAN` forever and it looks like a CI problem | Confirm it is only the review gate: `gh pr view <N> --json mergeStateStatus,reviewDecision` → `CLEAN` with an EMPTY `reviewDecision`. Then either merge with `--admin` (repo-admin override) or drop "require a review" from the ruleset if you are the only maintainer. ⚠️ `--admin` overrides branch protection, so some agent harnesses block it — if yours does, hand the merge to a human rather than routing around the guard |
| `gh pr merge --squash --delete-branch` prints a `failed to run git: ... would be overwritten by checkout` error | **The REMOTE merge already SUCCEEDED — only the LOCAL cleanup step failed.** `gh` merges via the API, then tries to switch your local checkout off the merged branch; a dirty working tree (often just CRLF-vs-LF churn from a script that rewrote a file) aborts that last step | **Check the remote before you retry** — `gh pr view <N> --json state,mergedAt,mergeCommit` and whether the issue closed. Re-running the merge on an already-merged PR is confusing at best. Finish the cleanup by hand: confirm the content diff is empty (`git diff --stat` / `--ignore-cr-at-eol`), `git checkout -- <file>`, then delete the branch with `gh api -X DELETE repos/<owner>/<repo>/git/refs/heads/<branch>` if `git push --delete` hangs |
